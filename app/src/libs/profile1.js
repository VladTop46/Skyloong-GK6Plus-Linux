        ...
        const macroQueue = macroData.TaskList.map(({ taskName, taskValue }) => {
            ...
            if (taskValue == '') {
                ...
                if (taskName == 'Delay') {
                    ...
                    if (taskValue == '"Left Shift"' || taskValue == '"Shift"') {
                        item.byData1 = 0x02;
                    }
                    else if (taskValue == '"Right Shift"') {
                        item.byData1 = 0x20;
                    }
                    else if (taskValue == '"Left Ctrl"' || taskValue == '"Ctrl"') {
                        item.byData1 = 0x01;
                    }
                    else if (taskValue == '"Right Ctrl"') {
                        item.byData1 = 0x10;
                    }
                    else if (taskValue == '"Left Alt"' || taskValue == '"Alt"') {
                        item.byData1 = 0x04;
                    }
                    else if (taskValue == '"Right Alt"') {
                        item.byData1 = 0x40;
                    }
                    else if (taskValue == '"Left Win"' || taskValue == '"Win"') {
                        item.byData1 = 0x08;
                    }
               ...
               // console.log('### write fnKey defined',byFnKeyDefine);
                for (let i = 0; i < byFnKeyDefine.length; i += 56) {
                    if (i + 56 <= byFnKeyDefine.length) {
                     ...
                     const {ack} =  await device.request(CMD.writeFnData(byModel, i, byFnKeyDefine.slice(i, i + 56)));
                        //console.log('write FnKeyDefine data 1',ack);
                    } else {

                      const {ack} =  await device.request(CMD.writeFnData(byModel, i, byFnKeyDefine.slice(i)));
